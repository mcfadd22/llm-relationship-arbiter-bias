...
**Placeholder legend:** `{agent}`/`{partner}` -- names; `{agent_subj}`/`{agent_subj_cap}`/`{agent_obj}`/`{agent_poss}` -- agent pronouns (subject/subject-capitalized/object/possessive); `{pronoun_partner_subj}`/`{pronoun_partner_obj}`/`{pronoun_partner_poss}` -- partner pronouns.
