...
3. **Falsifiable extension of the ambiguity mechanism found in RQ2 data** (see `project/project_status_summary.md`'s obligation_source-ambiguity finding): if hedging is driven by the same mechanism as the rating-gap-by-obligation_source pattern, hedge rate should be *elevated* in the same low-confidence categories (`fair_notice_of_expectations`, `good_faith_relationship_maintenance`) and *lowest* in `contribution_based_reciprocity`. Stated here, before the pilot, so it can't be fit to the data afterward. **Caveat added 2026-08-18:** the r=-0.82 obligation_source-level correlation this cites did not replicate at the individual-pair level (see `project/project_status_summary.md` and `analysis/confidence_ambiguity_findings.md`) -- it looks like an ecological-correlation artifact of the self-reported `confidence` field, which is heavily ceiling-compressed. This hedge-rate prediction is still worth stating as pre-registered since it's a different DV (hedging behavior, not rating dispersion) and doesn't depend on the self-report confidence field being valid, but don't treat the underlying "ambiguity effect" as an established mechanism going in.
