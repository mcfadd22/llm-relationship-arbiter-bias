# Partner-Identity Effect Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `scripts/analyze_partner_identity_effect.py` (does the partner's gender identity change how the agent is judged, holding agent gender constant?) and cross-reference it from `scripts/analyze_fault_rating_bias.py`'s older, weaker unpaired partner-gender finding, per `docs/superpowers/specs/2026-08-21-partner-identity-effect-design.md`.

**Architecture:** A precise structural mirror of `scripts/analyze_agent_identity_effect.py`'s Section A + omnibus, with agent/partner roles swapped (`build_partner_held_cells` becomes `build_agent_held_cells`), reusing the already-corrected within-cell-shuffle `cell_centered_omnibus` logic verbatim rather than reintroducing the global-shuffle bug that methodology fixed in commit `80b75db`. No same-identity ("Section B") equivalent -- that's a different question, already answered by item 4, and this script's docstring and output both explain why explicitly. One small text-only edit to the sibling script for cross-referencing.

**Tech Stack:** Python 3.11 stdlib only (`csv`, `glob`, `math`, `os`, `random`, `statistics`, `collections.defaultdict`) -- matches `analyze_agent_identity_effect.py` exactly, no new dependencies.

---

## Verification approach

Same convention as every prior script in this codebase: run end-to-end, sanity-check output. Task 1 has no prior committed number to reproduce (unlike item 2, which had the existing MM-vs-FF control to cross-check against) -- instead, verify plausibility (no NaN, no degenerate ranges) and structural consistency with the sibling script's own cell counts.

## Task 1: Build and run the partner-identity script

**Files:**
- Create: `scripts/analyze_partner_identity_effect.py`
...
def build_agent_held_cells(cells):
    """(scenario_id, severity, model, agent_gender) -> {partner_gender: row},
    pooling all three agent_gender values into separate extended cells --
...
Run: `cd /Users/mer_home/Documents/Me/Research/psych_bias_llms/Gender_AITA && source venv/bin/activate && python3 scripts/analyze_partner_identity_effect.py`
Expected: exits 0, prints `Wrote .../analysis/partner_identity_effect_findings.md`, and prints the full markdown report to stdout with no tracebacks. This runs the corrected `cell_centered_omnibus` (within-cell shuffles across ~2400 cells x 20000 permutations), so expect roughly the same runtime as `analyze_agent_identity_effect.py`'s corrected version (~1-2 minutes) -- run in the background if it looks like it will exceed a couple of minutes.
...
python3 -c "
import re
with open('analysis/partner_identity_effect_findings.md') as f:
    text = f.read()
assert not re.search(r'\bnan\b', text, re.IGNORECASE), 'found a NaN in the output'
assert text.count('vs') >= 6, 'expected at least 3 pairwise comparison blocks plus per-model headers'
print('basic sanity checks passed')
print()
print(text[:2000])
"
```
Expected: `basic sanity checks passed`, followed by the start of the report. Read the printed excerpt and confirm the three pairwise comparisons (M vs F, M vs NB, F vs NB) show plausible `n` values -- each should be in the same order of magnitude as `analysis/agent_identity_effect_findings.md`'s Section A comparisons (roughly 2400-2430, since it's the same underlying cells just re-grouped by agent_gender instead of partner_gender). If any comparison's `n` is drastically smaller (e.g. under 1000) or the omnibus `n_complete` looks wrong, stop and investigate the cell-building logic before proceeding -- don't paper over a structural bug.

- [ ] **Step 4: Commit**

```bash
git add scripts/analyze_partner_identity_effect.py analysis/partner_identity_effect_findings.md
git commit -m "$(cat <<'EOF'
Add partner-identity effect analysis (item 3)

Mirrors analyze_agent_identity_effect.py's Section A + omnibus with
agent/partner roles swapped: does the partner's gender identity
change how the agent is judged, holding agent gender constant?
Reuses the already-corrected within-cell-shuffle permutation logic
from commit 80b75db verbatim. Explicitly documents why there is no
NB-NB comparison here -- that's item 4's same-identity relationship
question, not this partner-identity question.
EOF
)"
```

## Task 2: Cross-reference the superseded finding

**Files:**
- Modify: `scripts/analyze_fault_rating_bias.py` (around line 630-633)

- [ ] **Step 1: Add the cross-reference note**

Find this exact text in `scripts/analyze_fault_rating_bias.py`:
```python
...
```python
    ...
    out.append(f"partner=M mean={statistics.mean([r['fault_rating'] for r in partner_groups['M']]):.3f}, "
                f"partner=F mean={statistics.mean([r['fault_rating'] for r in partner_groups['F']]):.3f}, "
                f"diff={diff:+.3f}, d={d:+.3f}, Welch t={t:.2f}.")
    out.append("**Superseded by a matched version**: this is an unpaired comparison "
                "pooling all rows, weaker than the paired designs used elsewhere in "
                "this project. `analysis/partner_identity_effect_findings.md` has the "
                "matched (agent-gender-held-constant) version of this comparison, "
                "extended to include the NB partner level -- cite that as the "
                "authoritative version; this unpaired number is kept here for "
                "continuity, not as the primary result.\n")
```
This is a text-only change -- the two `statistics.mean(...)` calls, `diff`, `d`, and `t` values are unchanged; only the appended markdown text after them changes.
...
Expected: exits 0, no tracebacks (this script's full run includes the reasoning-text correlation section, so it also depends on `analysis/reasoning_features.csv` already existing, which it does from earlier in this project).

- [ ] **Step 3: Confirm this was a pure text addition**

Run: `git diff analysis/fault_rating_bias_findings.md`
Expected: the only diff is the new "Superseded by a matched version" paragraph appearing right after the existing partner-gender-effect numbers. If any other number in the file changed, something about this edit affected an earlier code path -- stop and investigate; this was meant to be a pure text addition with no computation change.

- [ ] **Step 4: Commit**

```bash
git add scripts/analyze_fault_rating_bias.py analysis/fault_rating_bias_findings.md
git commit -m "Cross-reference the new matched partner-identity test from the older unpaired finding"
```

## Task 3: Document and update tracking

**Files:**
- Modify: `README.md` (Analysis section)
- Modify: `docs/planned_analysis.md` (item 3 entry and summary table row)

- [ ] **Step 1: Add a README entry**

In the `## Analysis` section of `README.md`, after the `scripts/analyze_partner_identity_effect.py`... (i.e. after whichever of `analyze_agent_identity_effect.py` / `score_scenario_sexism_content.py` bullets already exist -- insert after the last one in that section), insert:
```markdown
- **`scripts/analyze_partner_identity_effect.py`** -- reads
  `responses/confirmatory/*.csv`, tests whether the partner's gender
  identity (M/F/NB) changes how the agent is judged, holding agent gender
  constant -- the mirror image of `analyze_agent_identity_effect.py`'s
  agent-identity test. Writes `analysis/partner_identity_effect_findings.md`.
  Supersedes the older unpaired partner-gender finding in
  `analyze_fault_rating_bias.py`. See
  `docs/superpowers/specs/2026-08-21-partner-identity-effect-design.md` for
  why there is no NB-NB comparison in this script.
```

- [ ] **Step 2: Update item 3's entry in `docs/planned_analysis.md`**

Before editing, get the real numbers: run
`grep -A3 "Section A (partner identity)" analysis/partner_identity_effect_findings.md`
and read off the actual `n=`, `F(...)=`, and `p=` values, plus read the
three pairwise comparison lines from the same file for the M-vs-F, M-vs-NB,
and F-vs-NB `d_z` values. **Use those literal numbers below -- do not
write placeholder text into the file.**

Find the "## 3. Partner-gender secondary effect, extended to NB partner"
section in `docs/planned_analysis.md` and add a paragraph at the end of it
(after the existing `[needs new code]` line) with the real values you just
read, in the same style as item 2's write-up (which reports each pairwise
`d_z` value and a one-sentence interpretation of the pattern).

- [ ] **Step 3: Update the summary table row for item 3**
...
```
Replace the last cell with `**implemented 2026-08-21**, `scripts/analyze_partner_identity_effect.py`` followed by a one-line summary of the actual pairwise results (matching the style already used for items 2, 4, and 7's rows in this same table).

- [ ] **Step 4: Commit**

```bash
git add README.md docs/planned_analysis.md
git commit -m "Document analyze_partner_identity_effect.py and update item 3's tracked status"
```
